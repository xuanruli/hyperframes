...
worker(){
  ...
  local dir; dir=$(find /tmp/cc-all/ -maxdepth 1 -type d -name "${pre}*"|head -1)
  ...
  for c in $(pgrep -P "$p" 2>/dev/null); do kill -9 "$c" 2>/dev/null; done; kill -9 "$p" 2>/dev/null
  pkill -f "we-render.mjs $dir" 2>/dev/null; wait "$p" 2>/dev/null
  if grep -q "drawElement canvas injected" "$log" 2>/dev/null; then
    echo "$pre drawelement" > "$OUT/$pre.txt"
  else
    local reason; reason=$(grep -oE "falling back to (screenshot|beginframe) capture — [^(]*" "$log" | head -1 | sed -E 's/.*— //')
    ...
    if [ -z "$reason" ]; then
      reason=$(grep -oE "Fast capture: composition uses [a-z0-9 -]*" "$log" | head -1 | sed -E 's/.*uses /compile:/')
    fi
...
export -f worker
awk '{print $1}' "$SAMPLE" | xargs -P "$PAR" -I{} bash -c 'worker "$@"' _ {}
